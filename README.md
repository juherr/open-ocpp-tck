...
scenarios, and 11 of the 147 OCPP 2.0.1 cases the selection rule owes.

It brings a real charge point (the [`ocpp-cp-simulator`][sim] CLI, in a
digest-pinned container), drives it through 58 certification scenarios, parses
...
where all 147 are written down: 11 implemented, 136 declined against 23 reasons.
...
```
    ...
    ocpp-cp-simulator  <--- JSON Lines stdin ----|   ocpp-tck   |
      (docker, pinned)  ---- stdout wire log --->|   58 specs   |
                                                 +--------------+
```
...
`run-all` is the whole suite: all 58 scenarios, one command. It did not use to
...
| `ocpp-tck list-scenarios [--json]` | nothing | The 58 registered scenarios |

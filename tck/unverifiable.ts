...
import { UNVERIFIABLE_PREFIX as SENTINEL } from "./assert";

/** Marks a value as unanswerable by this driver, carrying the reason. */
export function unverifiable(reason: string): string {
  return `${SENTINEL}${reason}`;
}

export function isUnverifiable(value: string): boolean {
  return value.startsWith(SENTINEL);
}

/** The reason out of a sentinel value, or "" if it is not one. */
export function unverifiableReason(value: string): string {
  return isUnverifiable(value) ? value.slice(SENTINEL.length) : "";
}

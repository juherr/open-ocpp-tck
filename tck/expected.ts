...
export type ExpectedFailureTable = Readonly<
  Record<string, ExpectedFailureEntry>
>;

export function expectedFailureFor(
  table: ExpectedFailureTable | undefined,
  templateId: string,
): ExpectedFailureEntry | undefined {
 ...
 */
export function expectedFailureCoverage(
  table: ExpectedFailureTable,
  scope: ScopeTable | undefined,
  ...
  return {
    stale: ids.filter((id) => !registered.has(id)),
    notApplicable: ids.filter(
      (id) => scope?.[id]?.status === "NOT_APPLICABLE",
    ),
    reasonless: ids.filter((id) => !table[id]?.reason?.trim()),
    findingless: ids.filter((id) => !table[id]?.finding?.trim()),
  };
}

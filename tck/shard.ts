...
export function parseShard(raw: string): Shard | string {
  const parts = raw.split("/");
  if (parts.length !== 2) {
    return `--shard wants k/n (for example 1/3), got ${JSON.stringify(raw)}`;
  }
  const [rawIndex, rawTotal] = parts;
  // Number() rather than parseInt: parseInt("2x") is 2, and a typo that
  // silently selects half the suite is the failure this whole file is about.
  const index = Number(rawIndex);
  const total = Number(rawTotal);
  if (!Number.isInteger(index) || !Number.isInteger(total)) {
    return `--shard wants two integers, got ${JSON.stringify(raw)}`;
  }
  if (total < 1) return `--shard needs at least one shard, got ${total}`;
  if (index < 1 || index > total) {
    return `--shard ${index}/${total} is out of range: k must be 1..${total}`;
  }
  return { index, total };
}
...
export function describeShard(
  ...
  return (
    `Shard ${shard.index} of ${shard.total} — ${selected} of ${total} scenario(s). ` +
    `THIS RUN IS A PARTITION, NOT A SWEEP: the other ${total - selected} ran in ` +
    `sibling jobs, or did not run at all. It is not coverage on its own.`
  );
}

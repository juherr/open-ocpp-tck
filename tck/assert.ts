 ...
 */
export const UNEXERCISED_PREFIX = "scenario does not exercise this:";
...
export class AssertRecorder {
  ...
  get unexercised(): number {
    return this.checks.filter(
      (c) => c.status === "SKIPPED" && c.detail?.startsWith(UNEXERCISED_PREFIX),
    ).length;
  }
...
export function tallyAnswers(
  ...
  for (const { frame: call, index } of calls) {
    ...
    if (!response) {
      ...
      if (index < lastResponseIndex) tally.unanswered++;
...
export function assertAllAnswered(
  ...
  const { total, errors, unanswered, outstanding } = tallyAnswers(
    frames,
    action,
    direction,
  );
  ...
  if (total < minimum) {
    rec.skip(
      description,
      `${UNEXERCISED_PREFIX} expected at least ${minimum} ${direction} CALL for action=${action}, found ${total}`,
    );
    return;
  }

  if (errors.length === 0 && unanswered === 0) {
    const stillOutstanding =
      outstanding > 0
        ? `; ${outstanding} still outstanding when the log ended`
        : "";
    rec.pass(
      `${description} (${total - outstanding}/${total} answered${stillOutstanding})`,
    );
    return;
  }
  ...
  if (errors.length > 0) {
    const rendered = errors.map(
      (e) => `${e.errorCode}: ${e.errorDescription} (uniqueId=${e.uniqueId})`,
    );
    parts.push(
      `${errors.length}/${total} answered with a CALLERROR -- ${rendered.join("; ")}`,
    );
  }
  if (unanswered > 0) {
    parts.push(
      `${unanswered}/${total} never answered (the log continued past them)`,
    );
  }

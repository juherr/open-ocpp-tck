...
export function correlate(
  records: readonly Correlatable[],
): Correlation {
  const answers: (number | undefined)[] = records.map(() => undefined);
  const answered = new Set<number>();
  const orphanResponses: number[] = [];

  for (let i = 0; i < records.length; i++) {
    const response = records[i];
    if (response.messageType === "CALL") continue;
  ...
  records.forEach((record, index) => {
    if (record.messageType === "CALL" && !answered.has(index)) {
      unansweredCalls.push(index);
    }
  });

  return { answers, unansweredCalls, orphanResponses };
}

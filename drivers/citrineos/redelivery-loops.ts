...
/**
 * The dispatch envelope, key order included.
 *
 * The order is this deployment's and is the reason the refusal above exists. It
 * is written as one pattern rather than as three lookups because the three
 * members must come from the SAME envelope: a line-oriented reader that found an
 * action here and a station there would pair a request with another request's
 * station and report a loop that no message is in.
 */
const ENVELOPE =
  /\{"origin":"csms","eventGroup":"[a-z]+","action":"([A-Za-z]+)","context":\{"ocppConnectionName":"([A-Za-z0-9_-]+)","correlationId":"([0-9a-f-]+)"/;
...
export type LoopScan =
  | { loops: RedeliveryLoop[]; envelopes: number }
  | { unreadable: string };
 ...
 * being.
 */
export async function findRedeliveryLoops(
  lines: Iterable<string> | AsyncIterable<string>,
  threshold: number = LOOP_THRESHOLD,
): Promise<LoopScan> {
  const seen = new Map<string, RedeliveryLoop>();
  let envelopes = 0;
  let anyLine = false;
  for await (const line of lines) {
    if (line.length > 0) anyLine = true;
    const found = ENVELOPE.exec(line);
    if (!found) continue;
    envelopes += 1;
    const [, action, station, correlationId] = found;
    const existing = seen.get(correlationId);
    if (existing) {
      existing.logged += 1;
      continue;
    }
    seen.set(correlationId, { action, station, correlationId, logged: 1 });
  }
  if (anyLine && envelopes === 0) {
    return {
      unreadable:
        "the log carries no recognisable dispatch envelope, so this reader " +
        "matched nothing rather than finding nothing -- check the envelope " +
        "pattern against the pinned CitrineOS image before trusting a green " +
        "answer from it",
    };
  }
  const loops = [...seen.values()]
    .filter((loop) => loop.logged > threshold)
    .sort((a, b) => b.logged - a.logged);
  return { loops, envelopes };
}

/** The report a caller prints. Kept out of the reader so the guard asserts the
 *  finding rather than its wording. */
export function describeLoops(scan: LoopScan): string {
  if ("unreadable" in scan) return `REFUSED: ${scan.unreadable}`;
  if (scan.loops.length === 0) {
    return `no redelivery loop in ${scan.envelopes} dispatched message(s)`;
  }
  const rows = scan.loops
    .map(
      (loop) =>
        `  ${String(loop.logged).padStart(8)}x ${loop.action} -> ${loop.station} (${loop.correlationId})`,
    )
    .join("\n");
  return (
    `${scan.loops.length} redelivery loop(s) in ${scan.envelopes} dispatched message(s).\n` +
    `Each one runs for the rest of the sweep and they accumulate; eleven of them\n` +
    `stopped the CSMS answering entirely (open-ocpp-tck#119).\n${rows}`
  );
}
...
if (import.meta.main) {
  ...
  if (!path) {
    console.error("usage: bun drivers/citrineos/redelivery-loops.ts <csms log>");
    process.exit(2);
  }
  ...
  // Read as a stream: the log this is pointed at was 1.5 GB the one time it
  // mattered, and a reader that only works on the healthy ones is no reader.
  const scan = await findRedeliveryLoops(streamLines(file));
  console.log(describeLoops(scan));
  if ("unreadable" in scan) process.exit(2);
  process.exit(scan.loops.length > 0 ? 0 : 1);
}

/** Lines of a file, without holding it in memory. */
async function* streamLines(file: ReturnType<typeof Bun.file>) {

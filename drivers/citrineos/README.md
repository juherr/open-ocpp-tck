...
| **`Blocked` is unreachable from the 1.6 `Authorize` path** *(see the row above for the mechanism)*. | **TC_023.3 fails**, deterministically: CitrineOS answers `{"idTagInfo":{"status":"Invalid"}}` where the scenario requires `Blocked`. Observed 3 runs out of 3. | Kept red on purpose, and declared: `expected.ts` makes it an `EXPECTED FAIL` so the job stays blocking, and an `UNEXPECTED PASS` the day it is fixed. |
